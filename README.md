...
Длина, вложенность и метрики Ruff (`C901`, `PLR0912`) сравниваются численно по правилу и полному имени функции. Уменьшение превышения, например с 15 до 14 ветвей при лимите 12, допускается. Перенос функции на другие строки не считается регрессией.
...
Собственная проверка учитывает `if`, циклы, `try`, `with`, `match` и поддерживаемые асинхронные варианты. `elif` не добавляет уровень, вложенные функции и методы проверяются отдельно. Правило Ruff `PLR1702` пока не используется: оно остаётся preview, не считает сам `match` уровнем вложенности и иначе обрабатывает вложенные функции.
...
- Глобальные метрики `C901` и `PLR0912`: изолированный запуск с лимитами из `pyproject.toml` Harness. Настройки целевого проекта и `noqa` не отключают эти ограничения.
...
Ограничения длины и вложенности находятся в:
...
Тесты используют стандартный `unittest` и установленный Ruff. Они проверяют границы метрик, числовые регрессии, правила длины и вложенности, независимость глобальных лимитов от настроек проекта и цикл `PreToolUse` / `PostToolUse`. Тестовые файлы и снимки создаются во временных каталогах.
